...
cdef extern from "jelfloader.h":
    ...
    jelfLoaderStatus_t jelfLoaderInit(jelfLoaderContext_t **ctx_ptr, FILE *fd, const char *name, const jelfLoaderEnv_t *env);
...
def jelf_loader_hash(fn: bytes, name_to_sign: bytes):
    ...
    jelfLoaderInit(&ctx, fd, name_to_sign, &jelf_loader_env);
    jelfLoaderLoad(ctx);
