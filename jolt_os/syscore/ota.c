...
static int ota_ymodem_write_wrapper( const void *data, unsigned int size, unsigned int nmemb, void *cookie );
...
esp_err_t jolt_ota_ymodem( int8_t *progress )
{
    ...
    binary_file_length = ymodem_receive_write( (void *)jolt_ota_handle, update_partition->size, NULL,
                                               &ota_ymodem_write_wrapper, progress );
...
esp_err_t jolt_ota_init_handle()
{
    ...
    if( configured != running ) {
        ESP_LOGW( TAG,
                  "Configured OTA boot partition at offset 0x%08x, "
