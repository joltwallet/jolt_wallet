...
static void add_job_to_queue( jolt_bg_job_t *job )
{
    ...
    if( !xQueueSend( job_queue, job, 0 ) ) { ESP_LOGE( TAG, "Failed to re-add bg job to the queue" ); }
}
...
esp_err_t jolt_bg_init()
{
    ...
    if( NULL == task_handle ) {
        BaseType_t res = xTaskCreate( bg_task, "background", CONFIG_JOLT_TASK_STACK_SIZE_BACKGROUND, NULL,
...
static void abort_cb( lv_obj_t *obj, lv_event_t event )
{
    if( LV_EVENT_CANCEL == event ) {
        ...
        switch( xQueueSend( queue, &signal, 0 ) ) {
            case pdPASS: break;
            default: ESP_LOGE( TAG, "Failed to send JOLT_BG_ABORT signal" ); break;
        }
    }
}
...
exit:
    if( input_queue != NULL ) { vQueueDelete( input_queue ); }
    return ESP_FAIL;
}
...
void *jolt_bg_get_param( jolt_bg_job_t *job ) { return job->param; }

lv_obj_t *jolt_bg_get_scr( jolt_bg_job_t *job ) { return job->scr; }
