...
{
    assert( param->idx < 24 );
    param->scr = jolt_gui_scr_bignum_create( gettext( JOLT_TEXT_RESTORE ), gettext( JOLT_TEXT_ENTER_MNEMONIC_WORD ),
                                             param->mapping[param->idx] + 1, -1 );
...
static int32_t jolt_mnemonic_restore_process( jolt_bg_job_t *job )
{
    ...
    switch( param->state ) {
        ...
        case MNEMONIC_RESTORE_24WORDS: {
            char *line = NULL;
            int16_t word_idx;
            if( NULL == param->scr ) { jolt_cmd_mnemonic_restore_num_create( param ); }
