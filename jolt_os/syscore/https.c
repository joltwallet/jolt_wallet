...
static int https_func( jolt_bg_job_t *bg_job )
{
    ...
    if( !job->running ) {
        ...
        if( pdFALSE == xSemaphoreTake( client_mutex, 0 ) ) { return 100; /* Try again in 100mS */ }
        job->running = true;
        /* Set HTTP Method */
    ...
    if( ESP_OK == err ) { ESP_LOGD( TAG, "https client perform success.\n%s\n", response ); }
...
exit:
    ...
    if( NULL != job->cb ) { job->cb( status_code, response, job->param, scr ); }
    free( job );
    return 0;
}
...
esp_err_t jolt_network_client_init( char *uri )
{
    ...
    /* Allocate a static copy of the uri */
    {
        ...
        if( NULL != local_uri ) { free( local_uri ); }
        local_uri = tmp;
    }
