...
static const char *TAG = "storage_atecc608a";

bool storage_atecc608a_startup()
{
#if 0
...
bool storage_atecc608a_exists_mnemonic()
{
#if 0
...
/**
 * Generate a new stretch key on the atecc608a that never leaves it.
 * Also generate a stretching secret to prevent some physical attacks from
 * directly deriving your PIN.
 */
void storage_atecc608a_stretch_init()
{
#if 0
...
void storage_atecc608a_stretch( uint256_t hash, int8_t *progress )
{
#if 0
    CONFIDENTIAL uint8_t stretch_secret[32];
...
void storage_atecc608a_set_mnemonic( const uint256_t bin, const uint256_t pin_hash )
{
#if 0
...
bool storage_atecc608a_get_mnemonic( uint256_t mnemonic, const uint256_t pin_hash )
{
#if 0
    ...
    // Attempt Authorization and Get atecc608a Secret
    ...
    storage_atecc608a_set_pin_last( counter + 1 );
...
uint32_t storage_atecc608a_get_pin_count()
{
#if 0
...
void storage_atecc608a_set_pin_count( uint32_t count )
{
    /* nothing; pin_count is inherently increased during get_mnemonic attempt */
}

uint32_t storage_atecc608a_get_pin_last()
{
#if 0
...
void storage_atecc608a_set_pin_last( uint32_t count )
{
    // Make sure count is always increasing
    uint32_t old_count;
    old_count = storage_atecc608a_get_pin_last();
    if( count > old_count ) { storage_set_u32( count, "secret", "last_success" ); }
}

bool storage_atecc608a_get_u8( uint8_t *value, const char *namespace, const char *key, uint8_t default_value )
{
    return storage_internal_get_u8( value, namespace, key, default_value );
}

bool storage_atecc608a_set_u8( uint8_t value, const char *namespace, const char *key )
{
    return storage_internal_set_u8( value, namespace, key );
}

bool storage_atecc608a_get_u16( uint16_t *value, const char *namespace, const char *key, uint16_t default_value )
{
    return storage_internal_get_u16( value, namespace, key, default_value );
}

bool storage_atecc608a_set_u16( uint16_t value, const char *namespace, const char *key )
{
    return storage_internal_set_u16( value, namespace, key );
}

bool storage_atecc608a_get_u32( uint32_t *value, const char *namespace, const char *key, uint32_t default_value )
{
    return storage_internal_get_u32( value, namespace, key, default_value );
}

bool storage_atecc608a_set_u32( uint32_t value, const char *namespace, const char *key )
{
    return storage_internal_set_u32( value, namespace, key );
}

bool storage_atecc608a_get_str( char *buf, size_t *required_size, const char *namespace, const char *key,
                                const char *default_value )
{
    return storage_internal_get_str( buf, required_size, namespace, key, default_value );
}

bool storage_atecc608a_set_str( const char *str, const char *namespace, const char *key )
{
    return storage_internal_set_str( str, namespace, key );
}

bool storage_atecc608a_get_blob( unsigned char *buf, size_t *required_size, const char *namespace, const char *key )
{
    return storage_internal_get_blob( buf, required_size, namespace, key );
}

bool storage_atecc608a_set_blob( const unsigned char *buf, size_t len, const char *namespace, const char *key )
{
    return storage_internal_set_blob( buf, len, namespace, key );
}

void storage_atecc608a_factory_reset()
{
#if 0
...
bool storage_atecc608a_erase_key( const char *namespace, const char *key )
{
