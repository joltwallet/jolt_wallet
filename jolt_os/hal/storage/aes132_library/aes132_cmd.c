...
uint8_t aes132_auth( const uint8_t *key, uint16_t key_id, struct aes132h_nonce_s *nonce )
{
    ...
    if( 254 == nonce->value[12] || nonce->valid == false ) {
        ...
        if( res ) { ESP_LOGE( TAG, "Error Refreshing nonce for mutual auth" ); }
    }
    ...
    res = aes132m_execute( cmd, mode, param1, param2, sizeof( in_mac ), in_mac, 0, NULL, 0, NULL, 0, NULL, tx_buffer,
                           rx_buffer );
    if( AES132_DEVICE_RETCODE_SUCCESS != res ) {
    ...
    if( AES132_DEVICE_RETCODE_SUCCESS == res ) { ESP_LOGI( TAG, "AES132 returned valid MAC" ); }
    else {
...
uint8_t aes132_blockread( uint8_t *data, const uint16_t address, const uint8_t count )
{
    ...
    param1 = address;
    param2 = count;
    res    = aes132m_execute( cmd, mode, param1, param2, 0, NULL, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
    if( !res ) { memcpy( data, &rx_buffer[AES132_RESPONSE_INDEX_DATA], count ); }
    return res;
}
...
uint8_t aes132_counter( const uint8_t *mac_key, uint32_t *count, uint8_t counter_id, struct aes132h_nonce_s *nonce )
{
    ...
    res = aes132m_execute( cmd, mode, param1, param2, 0, NULL, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
    if( res ) {
        ESP_LOGE( TAG, "Error executing Counter command. Return Code 0x%02X", res );
        nonce->valid = false;
        goto exit;
    }
    ...
    res = aes132h_mac_check_decrypt( &mac_check_decrypt_param );
    if( AES132_DEVICE_RETCODE_SUCCESS == res ) {
        ESP_LOGD( TAG, "Successfully verified MAC Counter Read on Key Slot %d", counter_id );
    }
    else {
        ESP_LOGE( TAG, "AES132 returned invalid MAC" );
        nonce->valid = false;
                        ...
                        struct aes132h_nonce_s *nonce )
{
    ...
    res = aes132m_execute( cmd, mode, param1, param2, len, in, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
    if( 0 == res ) {
        if( NULL != out_mac ) { memcpy( out_mac, &rx_buffer[AES132_RESPONSE_INDEX_DATA], 16 ); }
        if( NULL != out_data ) { memcpy( out_data, &rx_buffer[AES132_RESPONSE_INDEX_DATA + 16], len ); }
        else {
                         ...
                         struct aes132h_nonce_s *nonce )
{
    ...
    res = aes132m_execute( cmd, mode, param1, param2, sizeof( in_mac ), in_mac, sizeof( enc_key ), enc_key, 0, NULL, 0,
                           NULL, tx_buffer, rx_buffer );
    if( res ) {
...
uint8_t aes132_legacy( const uint8_t key_id, uint8_t *data )
{
    ...
    res    = aes132m_execute( cmd, mode, param1, param2, 16, data, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
    if( res ) {
...
uint8_t aes132_mac_count( uint8_t *count )
{
    ...
    param2 = 0x0000;
    res    = aes132m_execute( cmd, mode, param1, param2, 0, NULL, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
    if( AES132_FUNCTION_RETCODE_SUCCESS == res ) { *count = rx_buffer[AES132_RESPONSE_INDEX_DATA + 1]; }
    else {
    }
    return res;
}
...
uint8_t aes132_mac_incr( struct aes132h_nonce_s *nonce )
{
...
#if MAC_COUNT_LOCKSTEP_CHECK
    {
        ...
        if( device_count != *mac_count ) {
            ESP_LOGE( TAG,
                      "mac_count desyncronized before incrementing.\n"
                      "Device=%d; Local=%d",
...
uint8_t aes132_nonce( struct aes132h_nonce_s *nonce, const uint8_t *in_seed )
{
    ...
    res    = aes132m_execute( cmd, mode, param1, param2, 12, data, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
    if( res ) {
        /* Error */
        ESP_LOGE( TAG, "Nonce returncode: %02X", rx_buffer[AES132_RESPONSE_INDEX_RETURN_CODE] );
        nonce->valid = false;
    }
    else {
        /* Success */
        ...
        ESP_LOGI( TAG,
                  "Local Nonce updated to %02X %02X %02X %02X %02X "
                  "%02X %02X %02X %02X %02X %02X %02X",
                  nonce->value[0], nonce->value[1], nonce->value[2], nonce->value[3], nonce->value[4], nonce->value[5],
                  nonce->value[6], nonce->value[7], nonce->value[8], nonce->value[9], nonce->value[10],
                  nonce->value[11] );
        if( mode & 0x01 ) {
...
uint8_t aes132_rand( uint8_t *out, const size_t n_bytes )
{
    ...
    do {
        res = aes132m_execute( cmd, mode, param1, param2, 0, NULL, 0, NULL, 0, NULL, 0, NULL, tx_buffer, rx_buffer );
        if( AES132_FUNCTION_RETCODE_SUCCESS != res ) { goto exit; }
        if( res ) { return res; }

        // Copy over up to 16 bytes
        if( n_bytes - out_head > 16 ) { n_copy = 16; }
        else {
            n_copy = n_bytes - out_head;
        }
        for( uint8_t i = 0; i < n_copy; i++ ) { out[out_head + i] ^= rx_buffer[AES132_RESPONSE_INDEX_DATA + i]; }
        out_head += n_copy;
    } while( n_copy > 0 );
