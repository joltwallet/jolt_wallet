...
static uint8_t i2c_master_cmd_begin_s( i2c_port_t i2c_num, i2c_cmd_handle_t cmd, TickType_t ticks_to_wait )
{
    uint8_t res = I2C_FUNCTION_RETCODE_COMM_FAIL;
    esp_err_t i2c_res;
    I2C_CTX { i2c_res = i2c_master_cmd_begin( i2c_num, cmd, ticks_to_wait ); }
    ...
    switch( i2c_res ) {
        case ESP_OK:
            ...
            break;
        case ESP_ERR_INVALID_ARG: ESP_LOGW( TAG, "i2c_master_cmd_begin parameter error." ); break;
        case ESP_FAIL:
            ...
            break;
        case ESP_ERR_INVALID_STATE: ESP_LOGE( TAG, "I2C driver not installed or not in master mode." ); break;
...
void i2c_disable_phys( void ) { i2c_driver_delete( CONFIG_JOLT_I2C_MASTER_NUM ); }
...
uint8_t i2c_send_bytes( uint8_t count, uint8_t *data )
{
     ...
     * 3-N) Data
     * N+1, N+2) CRC */
    for( uint8_t i = 0; i < count; i++ ) { ESP_LOGD( TAG, "Send byte %d: %.2X", i, data[i] ); }
 ...
 * \return status of the operation
 */
uint8_t i2c_receive_byte( uint8_t *data, const uint8_t *address ) { return i2c_receive_bytes( 1, data, address ); }
...
uint8_t i2c_receive_bytes( uint8_t count, uint8_t *data, const uint8_t *address )
{
    ...
    ESP_LOGD( TAG, "Performing read from memory address %.2X %.2X", address[0], address[1] );
    uint8_t res = I2C_FUNCTION_RETCODE_COMM_FAIL;
    i2c_cmd_handle_t cmd;
    if( NULL == ( cmd = i2c_cmd_link_create() ) ) {
        ESP_LOGE( TAG, "Failed to create i2c_cmd_link" );
    ...
    res = i2c_master_cmd_begin_s( CONFIG_JOLT_I2C_MASTER_NUM, cmd, 1000 / portTICK_RATE_MS );
    for( uint8_t i = 0; i < count; i++ ) { ESP_LOGD( TAG, "Received byte %d: %.2X", i, data[i] ); }
failure:
    i2c_cmd_link_delete( cmd );
    return res;
}
