...
static spp_receive_data_buff_t SppRecvDataBuff = {.node_num = 0, .buf_size = 0, .first_node = NULL};
...
{
    p1 = (spp_receive_data_node_t *)malloc( sizeof( spp_receive_data_node_t ) );
    if( p1 == NULL ) {
        ESP_LOGI( TAG, "malloc error %s %d\n", __func__, __LINE__ );
        return false;
    }

    if( p2 != NULL ) { p2->next_node = p1; }
