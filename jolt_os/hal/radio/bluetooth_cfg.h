 ...
 *
 * For most types of data in the GATT hierarchy, it is important to differentiate between their definition (the whole
 * group of attributes that make it up) and the declaration. The declaration is a single attribute that is always
 * placed first (in increasing handle order) within the definition and that introduces most of the metadata about the
 * data that follows. All declarations have read-only permissions with no security required, because they cannot
 * contain sensitive data. They are only structural attributes that allow the client to find out and discover the
 * layout and nature of the attributes on the server.
 *
