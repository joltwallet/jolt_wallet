...
static void delete_file_cb( lv_obj_t *btn, lv_event_t event )
{
    if( jolt_gui_event.short_clicked == event ) {
        ...
        jolt_gui_scr_del( scr_file_options );
        {
            ...
            if( len > 1 ) { jolt_gui_scr_menu_remove( scr_files, btn_file ); }
            else {
...
{
    if( jolt_gui_event.short_clicked == event ) {
        jolt_gui_scr_yesno_create( gettext( JOLT_TEXT_STORAGE_DETAILS ), delete_file_cb, NULL );
    }
}
...
{
    if( jolt_gui_event.short_clicked == event ) { scr_files = create_and_add_files_to_menu(); }
}
