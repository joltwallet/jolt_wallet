...
static uint8_t led_brightness_get() {
    uint8_t val;
    storage_get_u8(&val, "user", "led_val", 70 );
    return val;
}
...
static lv_res_t led_brightness_back(lv_obj_t *btn) {
    jolt_gui_scr_del();
    uint8_t val = led_brightness_get();
    jolt_led_set(val);
    return LV_RES_INV;
}
...
lv_res_t menu_led_brightness_create() {
    ...
    jolt_gui_scr_set_back_action(scr, led_brightness_back);
