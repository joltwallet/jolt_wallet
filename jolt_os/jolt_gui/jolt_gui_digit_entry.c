...
#define CHECK_SCR( x ) \
    if( JOLT_GUI_SCR_ID_DIGIT_ENTRY != jolt_gui_scr_id_get( x ) ) abort();
...
static jolt_gui_obj_t *digit_create( jolt_gui_obj_t *parent )
{
    ...
    JOLT_GUI_CTX
    {
        ...
        if( NULL == old_roller_signal ) { old_roller_signal = lv_obj_get_signal_cb( roller ); }
...
jolt_gui_obj_t *jolt_gui_scr_digit_entry_create( const char *title, int8_t n, int8_t pos )
{
    ...
    if( n > CONFIG_JOLT_GUI_SCR_DIGIT_ENTRY_MAX_LEN || n < 1 ) { return NULL; }
    ...
    JOLT_GUI_SCR_CTX( title )
    {
        ...
        if( 0 == pos ) { ext->decimal_point = BREAK_IF_NULL( create_dp( digit_entry ) ); }

        if( NULL == old_cont_signal ) { old_cont_signal = lv_obj_get_signal_cb( digit_entry ); }
...
int8_t jolt_gui_obj_digit_entry_get_arr( jolt_gui_obj_t *digit_entry, uint8_t *arr, uint8_t arr_len )
{
    ...
    if( n_entries > 0 ) {
        ...
        for( uint8_t i = 0; i < ext->num_rollers; i++ ) { printf( "%d ", arr[i] ); }
        printf( "\n" );
    }
#endif
    return n_entries;
...
double jolt_gui_obj_digit_entry_get_double( jolt_gui_obj_t *digit_entry )
{
    ...
    JOLT_GUI_CTX
    {
        ...
        for( int8_t i = ext->decimal_point_pos; i > 0; i-- ) { res /= 10; }
    }
    return res;
}
...
int32_t jolt_gui_obj_digit_entry_get_int( jolt_gui_obj_t *digit_entry )
{
    ...
    JOLT_GUI_CTX
    {
        ...
        for( uint8_t i = 0; i < n; i++ ) { res = concat_int( res, array[i] ); }
...
void jolt_gui_scr_digit_entry_set_pos( jolt_gui_obj_t *scr, int8_t pos )
{
    JOLT_GUI_CTX
    {
        ...
        digit_entry                 = JOLT_GUI_FIND_AND_CHECK( cont_body, JOLT_GUI_OBJ_ID_DIGIT_ENTRY );
        digit_entry_cont_ext_t *ext = lv_obj_get_ext_attr( digit_entry );
        if( pos < 0 || pos - 1 > ext->num_rollers ) { break; }
        lv_roller_set_visible_row_count( ext->rollers[ext->sel], 1 );
