...
{ 
    ...
    ctx.type = JOLT_CRYPTO_ED25519;
    ctx.private = private;
    ctx.private_len = sizeof(private);
    ctx.public = public;
    ctx.public_len = sizeof(public);

    {
        /* Populate Private Key */
        const char hex[] = "9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60";
        int res;
        res = sodium_hex2bin( ctx.private, ctx.private_len, hex, strlen( hex ), NULL, NULL, NULL );
        TEST_ASSERT_EQUAL_INT( 0, res );
    }

    /* Test Public Key Derivation */
    jolt_crypto_derive( &ctx );
    {
        ...
        TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_public, ctx.public, 32);
    }

#if 0
    /* Test Computed Signature */
    jolt_crypto_sign_init( &ctx );
    jolt_crypto_sign_update( &ctx, msg, sizeof(msg));
    jolt_crypto_sign_final( &ctx );

    {
        ...
        TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_signature, ctx.signature, 64);
    }
